...
const Household = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      ...
      try {
        ...
        // Lọc sản phẩm có idType === 3 (đồ gia dụng)
        const householdProducts = allProducts.filter(p => String(p.idType) === '4' || p.idType === 3);
  ...
  return (
    <div style={{background:'#fff', minHeight:'100vh', padding:'0', fontFamily:'inherit'}}>
      {/* Breadcrumb */}
      <div style={{fontSize:'1rem', color:'#888', padding:'32px 0 0 60px'}}>Shop All / Household Essentials</div>
      {/* Title */}
      <div style={{fontSize:'2.5rem', fontWeight:700, margin:'8px 0 32px 60px'}}>Household Essentials</div>
      <div style={{display:'flex', alignItems:'flex-start', maxWidth:1500, margin:'0 auto', padding:'0 32px'}}>
        ...
        <aside style={{width:260, minWidth:200, marginRight:32}}>
          ...
          <div style={{marginBottom:16}}>
            <span style={{background:'#f5f5f5', borderRadius:16, padding:'4px 12px', fontSize:'0.98rem', marginRight:8}}>Household ×</span>
          ...
          <div>
            ...
            <div style={{marginBottom:6}}><input type="checkbox" id="cleaning" disabled /> <label htmlFor="cleaning">Cleaning</label></div>
            <div><input type="checkbox" id="kitchen" disabled /> <label htmlFor="kitchen">Kitchen</label></div>
          </div>
        </aside>
        {/* Product grid */}
        <main style={{flex:1}}>
